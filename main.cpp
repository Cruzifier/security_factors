...
void cand_elim::negative_example(const str_vect & e) {
    vector<hypothesis> temp; // used to add back hypotheses that shouldn't be removed
    ...
    for (int i = 0; i < G.size(); ++i) {
        if (!(G[i] % e))    temp.push_back(G[i]);
        else {
            vector<hypothesis> new_g = G[i].min_specialise(e);
            for (int j = 0; j < new_g.size(); ++j) {
                //if (!(new_g[j] % e) && more_gen_than_S(new_g[j], true))
...
int main(int argc, char const *argv[]) {
        ...
        string filename = "C:\\Users\\Administrator\\CLionProjects\\security_factors\\cmake-build-debug\\training_sets\\table_1_data.txt";
        string outfile = "C:\\Users\\Administrator\\CLionProjects\\security_factors\\cmake-build-debug\\training_sets\\out.txt";
